...
def get_data_batch(data, batch_size, seed=0):
    """
    Extract a batch of data from the given dataset
    ...
    Returns:
        Numpy array of shape (batch_size, num_features)
    """
    np.random.seed(seed)
    ...
    if isinstance(data, pd.DataFrame):
        ...
        if batch_size > data.shape[0]:
            # If batch_size > data size, sample with replacement
            indices = np.random.choice(data.index, batch_size, replace=True)
        else:
            # Sample without replacement
            indices = np.random.choice(data.index, batch_size, replace=False)
        
        batch = data.loc[indices].copy()
        
        # ✅ Return numpy array with ALL columns (including Label if present)
        return batch.values
    else:
        # If data is already numpy array
        if batch_size > data.shape[0]:
            indices = np.random.choice(data.shape[0], batch_size, replace=True)
        else:
...
class RELEVAGAN_CC(gym.Env):
    ...
    def build_discriminator(self):
        ...
        model.add(LeakyReLU(negative_slope=0.2))  # ✅ Fix: Use negative_slope instead of alpha
        ...
        # ✅ Fix: Correct Input shape format
        img = Input(shape=(self.img_shape,))  # Note: comma makes it a tuple
    ...
    def train(self, model_components):
        ...
        # Directory
        os.makedirs(CACHE_PATH + TODAY, exist_ok=True)
        os.makedirs(FIGS_PATH + TODAY, exist_ok=True)
        epoch_number = 0
        log_iteration = 0
        if DEBUG:
        ...
        for i in range(starting_step, starting_step + nb_steps):
            ...
            for j in range(k_d):
                ...
                t_b = get_data_batch(shuffled_Bots, batch_size, seed=i + j)
                T_b = get_data_batch(shuffled_Normal, batch_size, seed=i + j)
                
                # ✅ Fix: Correct label and feature extraction
                if t_b.shape[1] == len(data_cols) + 1:  # Has Label column
                    labels = t_b[:, -1].astype(int)  # Last column is Label
                    t_b_features = t_b[:, :-1]  # All except last column
                else:
                    labels = np.zeros(batch_size, dtype=int)  # Default to bot label
                    t_b_features = t_b
                
                if T_b.shape[1] == len(data_cols) + 1:
                    Labels = T_b[:, -1].astype(int)  # Last column is Label  
                    T_b_features = T_b[:, :-1]  # All except last column
                else:
                    Labels = np.ones(batch_size, dtype=int)  # Default to normal label
                    T_b_features = T_b
                
                # ✅ Debug: Print shapes to verify
                print(f"t_b_features shape: {t_b_features.shape}, expected: ({batch_size}, {len(data_cols)})")
                print(f"T_b_features shape: {T_b_features.shape}, expected: ({batch_size}, {len(data_cols)})")
                
                if (self.minority_training_count < shuffled_Bots.shape[0] // batch_size + 1):
                    ...
                    # ✅ Use correct feature data (not t_b[:, :67])
                    d_l_B = self.discriminator.train_on_batch(
                        t_b_features, [real, labels, Labels]
                    )
            ...
            if i % log_interval == 0:  # 2x faster than testing each step...
                ...
                labels = X_for_plot[:, 68]
                G_Z_for_plot = self.generator.predict([z, labels])
                # g_z -= g_z.min()
                # g_z /= g_z.max()
                X_for_plot = X_for_plot[:, :68]
                ...
                labels = test_Bots["Label"].astype("int32").values.reshape(-1, 1)
                G_z = self.generator.predict([z, labels])
                ...
                G_Z_Real_acc_ = round(np.sum(pred_G_Z[0]) / G_Z.shape[0], 4)
                Ev_GZ_Bot = round(np.sum(pred_G_Z[1]) / G_Z.shape[0], 4)
# predict generated bot being real. If it maintains near 1 then it means it is bot.
                N_acc_ = round(np.mean(pred_Normal[2]), 4)# predict normal being normal. If it maintains near 1 then it means it is normal becasue normal has been labeled as 0.
                Ev_Real_Bot = round(np.mean(pred_Bots[1]), 4)  # predict bot being bot. If it maintains near 0 then it means it is bot because bot has been labeled as 1.
                ...
                if epoch_number % 10 == 0:
                    ...
                    POST_TRAINING_ESTIMATE_CLASSIFIERS = True
        ...
        if USE_RL_AT_THE_END:
            ...
            self.RL_TRAIN_BATCH = t_b[:, :68].copy()
            # print('self.RL_TRAIN_BATCH.shape[0]: ' + str(self.RL_TRAIN_BATCH.shape[0]))
