...
def prepare_cic_2017_data(PATH="", INPUT_FILE_NAME="", CSV_ONE_BOT=0):
    ...
    # ✅ FIX: Convert string labels to binary NGAY SAU KHI ĐỌC DATA
    print("Original label distribution:")
    print(data['Label'].value_counts())
    
    # Convert string labels to numeric: BENIGN = 0, attacks = 1  
    if data['Label'].dtype == 'object':
        print("Converting string labels to numeric...")
        data['Label'] = data['Label'].apply(lambda x: 0 if str(x).upper() == 'BENIGN' else 1)
        print("Conversion completed!")
    
    print("After conversion label distribution:")
    print(data['Label'].value_counts())

    # ✅ NOW the logic will work correctly
    normal = data.loc[data["Label"] == 0].copy()  # BENIGN
    bots = data.loc[data["Label"] == 1].copy()   # DDoS/Attacks
    ...
    positive_mask = (data_df >= 0).all(axis=1)
    data_df = data_df[positive_mask].copy()
    ...
    # Step 1: Create boolean mask for INF values
    inf_mask = np.isinf(data_df).any(axis=1)

# Step 2: Get indices of rows with INF values
    inf_indices = data_df.index[inf_mask]

    if DEBUG:
        print("INF values before removing: " + str(len(inf_indices)))

# Step 3: Remove INF rows if any exist
    if len(inf_indices) > 0:
        data_df = data_df.drop(inf_indices, axis=0).copy()
        data = data.drop(inf_indices, axis=0).copy()
        
        # Step 4: Update main dataframe
        data_cols = data_df.columns
        data[data_cols] = data_df.copy()
    ...
    # Check if there is any NaN
    # =============================================================================================
    nan_count = data_df.isnull().sum().sum()

    if DEBUG:
        print("NaN values: " + str(nan_count))

    # Check for any left over INF values - FIXED VERSION
    inf_mask = np.isinf(data_df).any(axis=1)
    inf_indices = data.index[inf_mask]

    if DEBUG:
        print("Any Left over INF values: " + str(len(inf_indices)))

    data[data_df.columns] = data_df.copy()

    # =============================================================================================
    # Handle network traffic specific processing
    # =============================================================================================
    if network_traffic:
        # Choose only Label column for labeling purposes in another data frame
        data_label_series = data["Label"].copy()
        
        # remove any columns with all values = Zero
        data = data.loc[:, (data != 0).any(axis=0)]
        
        if DEBUG:
            print("Data Columns after removing zero columns: " + str(data.columns))
    ...
    if DEBUG:
        print("Data Shape before final NULL and INF check: ")
        print(data.shape)
    ...
    if DEBUG:
        print("Data Shape after final NULL and INF removal: ")
        print(data.shape)
    ...
    normal = data.loc[data["Label"] == 0].copy()  # BENIGN
    bots = data.loc[data["Label"] == 1].copy()   # Attacks

    print("After Preprocessing: Total: " + str(data.shape))
    print("After Preprocessing: Normal: " + str(normal.shape))
    print("After Preprocessing: Bots: " + str(bots.shape))

    # =============================================================================================
    print("File: " + str(INPUT_FILE_NAME) + "_(Preprocessed).csv Saving ...")

    # Đảm bảo luôn lưu ở thư mục /kaggle/working/
    output_file = "/kaggle/working/" + str(INPUT_FILE_NAME) + "_(Preprocessed).csv"
