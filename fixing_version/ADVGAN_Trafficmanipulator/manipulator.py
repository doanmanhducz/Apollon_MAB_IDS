...
class Manipulator:
    ...
    def process(
        ...
        while True:
            ...
            STA_best_X, STA_best_feature, STA_best_pktList, STA_gbl_dis, STA_avg_dis, STA_best_all_feature, fe_time = pso.execute(
                last_end_time, groupList,
            ...
            if ed == len(self.pktList) or ed == limit:
                ...
                # Gộp tất cả packet đã được biến đổi
                all_adv_packets = []
                for pkt_group in STA_pktList_list:
                    all_adv_packets.extend(pkt_group)

                wrpcap("/kaggle/working/TrafficManipulator/adversaria10k_output.pcap", all_adv_packets)
                print(f"✅ Adversarial pcap saved! Total packets: {len(all_adv_packets)}")
